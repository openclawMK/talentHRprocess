...
async function runScoring(candidate, job) {
  ...
  const scoreObj = {
    ...
    screening_pass: scores.screening_pass,
    pre_interview_max: scores.pre_interview_max,
...
router.post("/assistant/ask", async (req, res) => {
  try {
    ...
    if (!(await hasTokens(companyId, "assistant"))) {
      return res.status(402).json({ error: "You've used all your AI assistant tokens. Contact PeopleQuest to add more." });
    }
    ...
    await consumeToken(companyId, "assistant");
    res.json({ answer });
  } catch (err) {
