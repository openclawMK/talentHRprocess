...
export default function InterviewScoring() {
  ...
  if (!candidate || !job) return <div style={{ background: D.cardBg, border: `0.5px solid ${D.border}`, borderRadius: 16, height: 280 }} className="animate-pulse" />;
  if (interviewDefs.length === 0) return (
    <div style={{ maxWidth: 720, margin: "0 auto" }}>
      <div onClick={() => navigate(`/jobs/${jobId}/candidate/${candidateId}`)} style={{ fontSize: 14, color: D.blue, fontWeight: 600, cursor: "pointer", marginBottom: 16 }}>← Back to candidate</div>
      <div style={{ textAlign: "center", color: D.text4, marginTop: 40 }}>No interview criteria defined for this role.</div>
    </div>
  );
  ...
  const back = (
    <div onClick={() => navigate(`/jobs/${jobId}/candidate/${candidateId}`)} style={{ fontSize: 14, color: D.blue, fontWeight: 600, cursor: "pointer", marginBottom: 16, display: "inline-flex", alignItems: "center", gap: 6 }}>← Back to candidate</div>
  );
    ...
    return (
      <div style={{ maxWidth: 940, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 13, marginBottom: 8 }}>
          ...
          <div>
            <h1 className="font-display" style={{ fontSize: 24, fontWeight: 800, letterSpacing: "-.5px", margin: 0, color: D.text }}>Interview Scoring</h1>
            <div style={{ fontSize: 14, color: D.text4, marginTop: 2 }}>Choose how to run this interview — you give the final score for each criterion.</div>
          </div>
        </div>

        {/* candidate strip */}
        <div style={{ display: "flex", alignItems: "center", gap: 14, background: D.cardBg, border: `0.5px solid ${D.border}`, borderRadius: 14, padding: "16px 20px", margin: "20px 0 24px" }} className="flex-wrap">
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: D.text }}>{name}</div>
            <div style={{ fontSize: 13, color: D.text4 }}>{job.role_title} · {stageLabel} stage · {round(candidate.score?.combined_score)} so far</div>
          </div>
          <div style={{ textAlign: "right" }}><div style={{ fontSize: 12, color: D.text4, fontWeight: 600 }}>Interview weight</div><div style={{ fontSize: 18, fontWeight: 800, color: "#7C3AED" }}>{interviewWeight}%</div></div>
          <div style={{ width: 1, height: 38, background: D.border }} />
          <div style={{ textAlign: "right" }}><div style={{ fontSize: 12, color: D.text4, fontWeight: 600 }}>Criteria</div><div style={{ fontSize: 18, fontWeight: 800, color: D.text }}>{interviewDefs.length}</div></div>
        </div>
        ...
        <div className="grid gap-[18px] md:grid-cols-2" style={{ marginBottom: 24 }}>
          ...
          <div onClick={chooseManual} style={{ background: D.cardBg, border: `1.5px solid ${D.border}`, borderRadius: 18, padding: 26, cursor: "pointer", display: "flex", flexDirection: "column" }} className="transition-all hover:-translate-y-0.5 hover:shadow-lg">
            ...
            <div style={{ fontSize: 18, fontWeight: 700, letterSpacing: "-.2px", marginBottom: 8, color: D.text }}>Write my own questions</div>
            <div style={{ fontSize: 14, color: D.text3, lineHeight: 1.55, marginBottom: 18 }}>Type your own questions for each criterion and score the candidate on them. Best if you already have a question set.</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 9, marginBottom: 22 }}>
              {["Full control over every question", "Reuse your team's standard set", "Score each criterion as you go"].map((t) => <div key={t} style={{ display: "flex", gap: 9, fontSize: 13.5, color: D.text2 }}><span style={{ color: "#4F46E5" }}>✓</span> {t}</div>)}
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "auto" }}>
              <span style={{ fontSize: 13, color: D.text4 }}>✍ Blank scoring sheet</span>
        ...
        {/* what you'll score */}
        <div style={{ background: D.cardBg, border: `0.5px solid ${D.border}`, borderRadius: 16, padding: 22 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: D.text }}>What you'll score</div>
            <div style={{ fontSize: 12.5, color: D.text4, fontWeight: 600 }}>From the role's criteria</div>
          </div>
          <div style={{ fontSize: 13, color: D.text4, marginBottom: 18 }}>Both methods score the same {interviewDefs.length} interview criteria — only the questions differ.</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {interviewDefs.map((c) => (
              <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 14, background: D.inset, borderRadius: 12, padding: "14px 16px" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 14.5, fontWeight: 700, color: D.text }}>{c.name}</div>{c.description && <div style={{ fontSize: 13, color: D.text4, marginTop: 2 }}>{c.description}</div>}</div>
  ...
  if (loadingAi) return (
    <div style={{ maxWidth: 720, margin: "0 auto" }}>
      {back}
      <div style={{ textAlign: "center", color: D.text4, marginTop: 60 }}>
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto" }} className="pb-4">
      {back}
      {/* dark header — intentionally always dark, matches the mockup's interview accent */}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {list.map((c, i) => {
          ...
          return (
            <div key={c.id} style={{ background: D.cardBg, border: `0.5px solid ${D.border}`, borderRadius: 16, padding: "22px 24px" }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 14, marginBottom: 18 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: ".6px", color: D.text5, textTransform: "uppercase", marginBottom: 6 }}>Criterion {i + 1} of {list.length}</div>
                  <div style={{ fontSize: 16.5, fontWeight: 700, letterSpacing: "-.2px", color: D.text }}>{c.name}</div>
                  {c.description && <div style={{ fontSize: 13, color: D.text4, marginTop: 3 }}>{c.description}</div>}
                </div>
              ...
              {mode === "manual" ? (
                <>
                  <div style={{ fontSize: 13, fontWeight: 700, color: D.text2, marginBottom: 8 }}>Your questions <span style={{ color: D.text4, fontWeight: 500 }}>(one per line)</span></div>
                  <textarea value={manualQ[c.id] || ""} onChange={(e) => setManualQ((q) => ({ ...q, [c.id]: e.target.value }))} placeholder={"e.g. Tell me about a time you handled a difficult customer.\nWhat would you do if two staff called in sick during a rush?"} style={{ width: "100%", minHeight: 84, padding: "13px 15px", border: `0.5px solid ${D.border}`, borderRadius: 11, fontSize: 14, color: D.text2, lineHeight: 1.6, resize: "vertical", background: D.inset, outline: "none" }} />
                </>
              ) : (
                <>
              ...
              <div style={{ marginTop: 20, paddingTop: 18, borderTop: `0.5px solid ${D.hair}` }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: D.text2 }}>Your rating</span>
                  <span style={{ fontSize: 13, color: D.text3 }}><b style={{ color: rm.badge, fontSize: 15 }}>{rating}</b> / 100</span>
                </div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, fontSize: 11.5, color: D.text5 }}><span>0 · Not demonstrated</span><span>50 · Adequate</span><span>100 · Excellent</span></div>
              </div>

              <textarea value={notes[c.id] || ""} onChange={(e) => setNotes((n) => ({ ...n, [c.id]: e.target.value }))} placeholder="Notes on candidate's answer (optional)…" style={{ width: "100%", minHeight: 54, padding: "12px 15px", border: `0.5px solid ${D.border}`, borderRadius: 11, fontSize: 14, color: D.text2, lineHeight: 1.6, resize: "vertical", background: D.inset, outline: "none", marginTop: 18 }} />
            </div>
          );
        })}
      </div>
      ...
      <div style={{ position: "sticky", bottom: 0, marginTop: 20, background: D.cardBg, backdropFilter: "blur(8px)", border: `0.5px solid ${D.border}`, borderRadius: 14, padding: "14px 18px", display: "flex", alignItems: "center", gap: 16 }} className="flex-wrap">
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 13, color: D.text4 }}>Overall</span>
          <span style={{ fontSize: 22, fontWeight: 800, color: om.color, letterSpacing: "-.5px" }}>{overall}</span>
          <span style={{ fontSize: 13, color: D.text4 }}>/ 100</span>
          <span style={{ fontSize: 11.5, fontWeight: 700, color: "#fff", background: om.color, padding: "3px 10px", borderRadius: 20 }}>{om.tag}</span>
        </div>
        <button onClick={() => navigate(`/jobs/${jobId}/candidate/${candidateId}`)} style={{ marginLeft: "auto", padding: "12px 18px", background: "transparent", color: D.text3, border: "none", fontWeight: 600, fontSize: 14.5, cursor: "pointer" }}>Cancel</button>
